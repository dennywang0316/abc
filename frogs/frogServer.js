...
io.on('connection', (socket) => {
    ...
    socket.on("my-role", function(data){
        if(data.role == "frog"){
            ...
            frogs.push(frogData);
            console.log(frogs); //printing out the frog array
           
            if(conductor){
                io.to(conductor).emit('new-frog', frogData);//离开socket level, 因为如果直接使用socket.emit,会send back to这个新login的client，所以用io.to去指定发送给conductor
            }
