...
function draw() {
  ...
  //runs only once to initiate the map
  if (!mapInit && GPS_GRANTED && currentLongitude != 0) {
    console.log("starting map");
    mappa_options.lat = currentLatitude; // zero at first, update them to current latitude
    mappa_options.lng = currentLongitude; // zero at first, update them to current longitude
...
function touchStarted() {
  if (mapInit) {
    let pos = myMap.pixelToLatLng(touches[0].x, touches[0].y); //sending the touch x,y to the lat and lng in the map
...
//directly called from GPS listener whenever our location update, get data from GPS receiver
