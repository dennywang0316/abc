...
io.on('connection', (socket) => {
    ...
    socket.on("identify", function(data){
        // console.log(data);
        // connect username and user id to socket ids （data management)
        sockets[socket.id] = {
            userId: data.userId,
    ...
    socket.on("message-from-client", function(data){
        ...
        let stringfiedMessage = JSON.stringify(messages, null, 2)
        fs.writeFileSync(DATA_PATH, stringfiedMessage, "utf-8"); //1:which file, 2:what to sync, 3:text encoding model
